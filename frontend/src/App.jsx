...
function App() {
  ...
  const lastReportedReceiverPct = useRef(-1);
  ...
  const setupDataChannelListeners = () => {
    ...
    dataChannelRef.current.bufferedAmountLowThreshold = 1024 * 1024 * 1;
    ...
    dataChannelRef.current.onmessage = (event) => {
      ...
      if (event.data instanceof ArrayBuffer) {
        ...
        if (pct !== lastReportedReceiverPct.current) {
          lastReportedReceiverPct.current = pct;
          const receivedMB = (byteReceivedCount.current / (1024 * 1024)).toFixed(1);
          const totalMB = (incomingFileMetadata.current.size / (1024 * 1024)).toFixed(1);
          setProgress(pct);
          setStatus(`Downloading: ${receivedMB} MB / ${totalMB} MB`);
        }
        ...
        if (byteReceivedCount.current >= incomingFileMetadata.current.size) {
          ...
          setProgress(0);
          lastReportedReceiverPct.current = -1;
        }
      }
    };
  };
